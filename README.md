...
- **Zero System Pollution** — All API hooks are process-scoped and removed on shutdown; no registry writes, no system file modifications, no traces left behind
- **Auto-Activation** — Automatically activates Flash content without user clicks, including Flash embedded in iframes
- **Local Data Isolation** — Flash Local Shared Objects (LSOs) and config files are redirected to a local `FlashData/` directory instead of `%APPDATA%`
...
4. **File System Hooks** — Redirect `%APPDATA%\Macromedia\Flash Player` and `%APPDATA%\Adobe\Flash Player` paths to a local `FlashData/` directory
5. **Activation Hooks** — Hook `IOleObject::SetClientSite` and `IQuickActivate::QuickActivate` vtables to force Flash in-place activation via a coalesced timer
...
The build automatically copies the architecture-matched `Flash.ocx` and `mms.cfg` to the output directory under `Flash/`.

## Usage

1. Build or download `FlashIE.exe`
2. Ensure the `Flash/` directory (containing `Flash.ocx` and `mms.cfg`) is alongside the executable
3. Run `FlashIE.exe`
4. Enter a URL or local file path containing Flash content in the address bar

Flash data (LSOs, etc.) will be stored in `FlashData/` next to the executable, not in your system's `%APPDATA%`.
...
flashie/
├── source/
│   ├── flash_loader.h/cpp    # API hooking engine (COM, registry, security, filesystem hooks)
...
│   ├── mms.cfg               # Flash config (disables EOL checks and auto-update)
