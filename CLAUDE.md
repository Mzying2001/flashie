...
- **`source/flash_loader.h/.cpp`** — `FlashLoader` class. Loads `Flash.ocx` from the exe directory via `LoadLibrary` + `DllGetClassObject`, registers its class factory into COM, then installs inline function hooks (detours) on COM APIs (`CoGetClassObject`, `CoCreateInstance`, `CoGetClassObjectFromURL`, `CLSIDFromProgID`), registry APIs (`RegOpenKeyExW`, `RegQueryValueExW`, `RegCloseKey`), Windows security APIs (`WldpIsClassInApprovedList`, `WldpQueryDynamicCodeTrust`, `CoInternetIsFeatureEnabled`), TypeLib loading (`LoadRegTypeLib`), and file system APIs (`CreateFileW`, `CreateDirectoryW`, etc.). The hooks intercept requests for the Flash CLSID (`{D27CDB6E-AE6D-11CF-96B8-444553540000}`) and redirect them to the locally-loaded factory, fake registry entries for Flash's InprocServer32/TypeLib/ProgID, bypass ActiveX kill bits and WLDP approval checks, redirect Flash data paths to a local `FlashData/` directory, and inject `IObjectSafety`/`allowScriptAccess` support for MSHTML scripting. Contains a minimal x86/x64 instruction length decoder for computing hook trampoline sizes.
