...
- **`source/browser.h/.cpp`** — `BrowserHost` and OLE site classes (`COleSite`, `COleClientSite`, `COleInPlaceSite`, `COleInPlaceFrame`). Implements the OLE container interfaces needed to host an `IWebBrowser2` (IE) control in-process. `COleSite` also implements `IInternetSecurityManager` (maps all URLs to the same zone and security ID to prevent cross-origin blocking), `IDocHostUIHandler`, and a `DWebBrowserEvents2` event sink. Navigation uses a two-phase reset pattern: navigates to `about:blank` first to clear the security context, then to the real URL on `DOCUMENTCOMPLETE`.
