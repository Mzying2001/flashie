...
- **`source/flash_loader.h/.cpp`** — `FlashLoader` class. The core hooking engine, organized into 8 sections:
  ...
  - **Section 7h**: SharedObject first-save compatibility — scoped `DeleteFileA/W` hooks treat `ERROR_FILE_NOT_FOUND` as success only when the local `Flash.ocx` deletes a `.sol`, allowing Flash to continue its own `.sxx` commit. All other file deletions pass through unchanged.
